...
func (this *Service) processor() {
	defer func() {
		...
		//log.Debug("(%s) Stopping processor", this.cid())
	}()

	log.Debug("(%s) Starting processor", this.cid())
	...
	for {
		...
		if err != nil {
			if err != errDisconnect {
				log.Error("(%s) Error processing %s: %v", this.cid(), msg.Name(), err)
			} else {
				return
			}
		}
		...
		if err != nil {
			if err != io.EOF {
				log.Error("(%s) Error committing %d read bytes: %v", this.cid(), total, err)
			}
			return
		}
		...
		//	log.Debug("(%s) Going to process message %d", this.cid(), this.inStat.msgs)
		//}
	}
}
...
func (this *Service) processIncoming(msg message.Message) error {
	...
	if err != nil {
		log.Debug("(%s) Error processing acked message: %v", this.cid(), err)
	}
	return err
}
...
func (this *Service) processAcked(ackq *sessions.Ackqueue) {
	for _, ackmsg := range ackq.Acked() {
		...
		if err != nil {
			log.Error("process/processAcked: Unable to creating new %s message: %v", ackmsg.Mtype, err)
			continue
		}

		if _, err := msg.Decode(ackmsg.Msgbuf); err != nil {
			log.Error("process/processAcked: Unable to decode %s message: %v", ackmsg.Mtype, err)
			continue
		}

		ack, err := ackmsg.State.New()
		if err != nil {
			log.Error("process/processAcked: Unable to creating new %s message: %v", ackmsg.State, err)
			continue
		}

		if _, err := ack.Decode(ackmsg.Ackbuf); err != nil {
			log.Error("process/processAcked: Unable to decode %s message: %v", ackmsg.State, err)
			continue
		}

		//log.Debug("(%s) Processing acked message: %v", this.cid(), ack)
		...
		case message.PUBREL:
			...
			if err = this.onPublish(msg.(*message.PublishMessage)); err != nil {
				log.Error("(%s) Error processing ack'ed %s message: %v", this.cid(), ackmsg.Mtype, err)
			}
		...
		default:
			log.Error("(%s) Invalid ack message type %s.", this.cid(), ackmsg.State)
			continue
		}
		...
		if ackmsg.OnComplete != nil {
			...
			if !ok {
				log.Error("process/processAcked: Error type asserting onComplete function: %v", reflect.TypeOf(ackmsg.OnComplete))
			} else if onComplete != nil {
				if err := onComplete(msg, ack, nil); err != nil {
					log.Error("process/processAcked: Error running onComplete(): %v", err)
				}
			}
		}
	}
...
func (this *Service) processSubscribe(msg *message.SubscribeMessage) (err error) {
	...
	for _, rm := range this.rmsgs {
		if err := this.Publish(rm, nil); err != nil {
			log.Error("Service/processSubscribe: Error publishing retained message: %v", err)
			return err
		}
	}
...
func (this *Service) onPublish(msg *message.PublishMessage) error {
	if msg.Retain() {
		if err := this.topicsMgr.Retain(msg); err != nil {
			log.Error("(%s) Error retaining message: %v", this.cid(), err)
		}
	}

	err := this.topicsMgr.Subscribers(msg.Topic(), msg.QoS(), &this.subs, &this.qoss)
	if err != nil {
		log.Error("(%s) Error retrieving subscribers list: %v", this.cid(), err)
		return err
	}
