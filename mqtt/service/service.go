...
func (this *Service) stop() {
	...
	if this.done != nil {
		log.Debug("(%s) closing this.done", this.cid())
		close(this.done)
	}

	// Close the network connection
	if this.conn != nil {
		log.Debug("(%s) closing this.conn", this.cid())
		this.conn.Close()
	}
	...
	if !this.client && this.sess != nil {
		...
		if err != nil {
			log.Error("(%s/%d): %v", this.cid(), this.id, err)
		} else {
			for _, t := range topics {
				if err := this.topicsMgr.Unsubscribe([]byte(t), &this.onpub); err != nil {
					log.Error("(%s): Error unsubscribing topic %q: %v", this.cid(), t, err)
				}
			}
		}
	}
