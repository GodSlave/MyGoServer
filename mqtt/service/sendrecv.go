...
func (this *Service) receiver() {
	...
	default:
		log.Error("(%s) %v", this.cid(), ErrInvalidConnectionType)
	}
}
...
func (this *Service) sender() {
	...
	case net.Conn:
		for {
			...
			if err != nil {
				if err != io.EOF {
					log.Error("(%s) error writing data: %v", this.cid(), err)
				}
				return
			}
		}
	...
	default:
		log.Error("(%s) Invalid connection type", this.cid())
	}
}
...
func (this *Service) readMessage(mtype message.MessageType, total int) (message.Message, int, error) {
	...
	for l < total {
		...
		l += n
		log.Debug("read %d bytes, total %d", n, l)
		if err != nil {
			return nil, 0, err
		}
	}
